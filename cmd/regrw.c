...
static int do_regrw(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	bool all = false;
	debug("Enter the config_dm_gpio!\n");
#endif
 ...
 show_usage:
	 ...
	 */
	debug("By_xingyanl the str_gpio is %s\n", str_gpio);
	ret = gpio_lookup_name(str_gpio, NULL, NULL, &gpio);
	debug("At the 180 lines by xingyanl \n");
...
U_BOOT_CMD(regrw, 4, 0, do_regrw,
	   "red and write gpio reg pins",
	   "<input|set|clear|toggle> <pin>\n"
	   "    - input/set/clear/toggle the specified pin\n"
	   "regrw status [-a] [<bank> | <pin>]  - show [all/claimed] GPIOs");
